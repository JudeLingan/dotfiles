...
loadxrdb()
{
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        XRDB_LOAD_COLOR("color12", normbordercolor);
        XRDB_LOAD_COLOR("background", normbgcolor);
        XRDB_LOAD_COLOR("color12", normfgcolor);
        XRDB_LOAD_COLOR("color14", selbordercolor);
        XRDB_LOAD_COLOR("background", selbgcolor);
        XRDB_LOAD_COLOR("color14", selfgcolor);
      }
    }
  }
...
col(Monitor *m)
{
	...
	if (n > m->nmaster)
		...
		mw = m->ww - m->gap->gappx;
	for (i = 0, x = y = m->gap->gappx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			w = (mw - x) / (MIN(n, m->nmaster) - i);
			resize(c, x + m->wx, m->wy + m->gap->gappx, w - (2 * c->bw), m->wh - (2 * c->bw) - 2*m->gap->gappx, 0);
			x += WIDTH(c) + m->gap->gappx;
		} else {
			h = (m->wh - y) / (n - i) - m->gap->gappx;
			resize(c, x + m->wx, m->wy + y, m->ww - x - (2 * c->bw) - m->gap->gappx, h - (2 * c->bw), 0);
			y += HEIGHT(c) + m->gap->gappx;
		}
}
